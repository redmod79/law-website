#!/usr/bin/env python3
"""
generate_simple_conclusions.py — Generate conclusion-simple.md for each law study.

Reads each study's CONCLUSION.md and uses the Anthropic API to produce
a plain-language summary following the etc-website pattern.
"""
...
def get_study_folders():
    """Find all law study folders in docs/studies/."""
    folders = []
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        folders = [f for f in folders if f.name in targets or
                   re.match(r"law-(\d{2})", f.name).group(0) in targets]
